...
const RoleSelector: React.FC<{ isOpen: boolean; onClose: () => void }> = ({
  isOpen,
  onClose,
}) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50 px-4">
      <div className="
        bg-[#eafbece9] rounded-lg shadow-lg p-6 
        w-full max-w-sm 
        sm:max-w-md 
        md:w-1/3
        relative
      ">
        <h2 className="text-3xl font-semibold text-center mb-2 mt-3 text-primary/95">
          Select your User Type
        </h2>

        <p className="text-center text-lg mb-6 font-medium">
          To create an account, please select your account type and continue to TerraSustain
        </p>
